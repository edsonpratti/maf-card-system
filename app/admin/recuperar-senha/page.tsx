...
export default function AdminRecuperarSenhaPage() {
    ...
    useEffect(() => {
        // O Supabase detecta automaticamente o token no hash da URL
        // e dispara o evento PASSWORD_RECOVERY quando válido
        const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
            if (event === "PASSWORD_RECOVERY") {
                setPageState("form")
            } else if (event === "SIGNED_IN") {
                // Pode chegar SIGNED_IN junto — só muda se ainda estiver carregando
                setPageState(prev => prev === "loading" ? "form" : prev)
            }
        })

        // Timeout: se em 5 segundos não detectar o token, mostra erro
